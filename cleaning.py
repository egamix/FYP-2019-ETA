...
for name, val in tqdm(grouper):
		...
		wz = int(30 * arctan(0.005*val.shape[0]))

		if (wz > 5):
			outliers = outlier_find(val, window_size=wz).astype(bool)
		else:
			outliers = np.zeros(val.shape[0]).astype(bool)

		val = val[~outliers]
		a = val.reset_index()
		a = a[['index', 'iso timestamp', 'latitude', 'longitude']].values
		ang = cosrule_arr(a[:,2:].astype(float))
		a = np.append(a, ang.reshape(-1,1), axis = 1)
		# a = np.append(a, np.array(val['iso timestamp'].values).reshape(-1,1), axis =1)
		while (a[:,4] < 30).any():
			a = a[~shift5(a[:,4] < 30, -1)] 
			a[:,4] = cosrule_arr(a[:,2:4].astype(float))

		val = val.loc[a[:,0],:]
		
		out.append(val)
		# a = pd.DataFrame(a)
		# a.columns = ['iso timestamp','latitude','longitude', 'angle']
		# a = compute_speed(a)
		# print(name)
		# # prod = []
		# for pair in itertools.combinations(val.iloc[:,0:].values,2):
		# 	prod.append(pair)
		# try:
